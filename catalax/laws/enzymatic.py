...
def irreversible_non_modulated_non_interacting_bireactant_enzymes(
    ...
    Returns:
        str: The kinetic law equation as a string
    """
    equation = sympify("(E_t*k_p*(S_a)/(K_1)*(S_b)/(K_2))/((1+(S_a)/(K_1))*(1+(S_b)/(K_2)))", locals=LOCALS)
...
def irreversible_non_modulated_non_interacting_trireactant_enzymes(
    ...
    Returns:
        str: The kinetic law equation as a string
    """
    equation = sympify("(E_t*k_p*(S_a)/(K_1)*(S_b)/(K_2)*(S_3)/(K_3))/((1+(S_a)/(K_1))*(1+(S_b)/(K_2))*(1+(S_3)/(K_3)))", locals=LOCALS)
...
def reversible_iso_uni_uni(
    ...
    Returns:
        str: The kinetic law equation as a string
    """
    equation = sympify("(V_f*(S-(P)/(K_eq)))/(S*(1+(P)/(K_i))+K_ms*(1+(P)/(K_mp)))", locals=LOCALS)
...
def unmodulated_reversible_hill_type(
    ...
    Returns:
        str: The kinetic law equation as a string
    """
    equation = sympify("(V_f*(S)/(K_s)*(1-(P)/(S*K_eq))*((S)/(K_s)+(P)/(K_p))^(h-1))/(1+((S)/(K_s)+(P)/(K_p))^h)", locals=LOCALS)
...
def ordered_bi_bi_mechanism(
    ...
    Returns:
        str: The kinetic law equation as a string
    """
    equation = sympify("(V_f*S_a*S_b-P_p*(P_q)/(K_eq))/(S_a*S_b*(1+(P_p)/(K_ip))+K_ma*S_b+K_mb*(S_a+K_ia)+(V_f)/(V_r*K_eq)*(K_mq*P_p*(1+(S_a)/(K_ia))+P_q*(K_mp*(1+(K_ma*S_b)/(K_ia*K_mb))+P_p*(1+(S_b)/(K_ib)))))", locals=LOCALS)
...
def ordered_bi_uni_mechanism(
    ...
    Returns:
        str: The kinetic law equation as a string
    """
    equation = sympify("(V_f*S_a*S_b-(P)/(K_eq))/(S_a*S_b+K_ma*S_b+K_mb*S_a+(V_f)/(V_r*K_eq)*(K_mp+P*(1+(S_a)/(K_ia))))", locals=LOCALS)
...
def ordered_uni_bi_mechanism(
    ...
    Returns:
        str: The kinetic law equation as a string
    """
    equation = sympify("(V_f*(S-(P_p*P_q)/(K_eq)))/(K_ms+S*(1+(P_p)/(K_ip))+(V_f)/(V_r*K_eq)*(K_mq*P_p+K_mp*P_q+P_p*P_q))", locals=LOCALS)
...
def ping_pong_bi_bi_mechanism(
    ...
    Returns:
        str: The kinetic law equation as a string
    """
    equation = sympify("(V_f*(S_a*S_b-(P_p*P_q)/(K_eq)))/(S_a*S_b+K_mb*S_a+K_ma*S_b*(1+(P_q)/(K_iq))+(V_f)/(V_r*K_eq)*(K_mq*P_p*(1+(S_a)/(K_ia))+P_q*(K_mp+P_p)))", locals=LOCALS)
