...
Understanding the structure of Catalax datasets is essential for effectively working with simulation results and experimental data. A `Dataset` serves as a container that organizes related measurements and provides a consistent interface for analysis and visualization.

A `Dataset` contains several key components:
- **`species`**: A list of chemical species names that defines what molecules are tracked in this dataset
- **`measurements`**: A list of `Measurement` objects, where each measurement represents one experimental condition or simulation run
- **`name`, `description`**: Metadata fields that help organize and document your datasets for reproducibility and sharing

Each individual `Measurement` within a dataset contains the detailed information for one experimental condition:
- **`initial_conditions`**: A dictionary mapping species names to their initial concentrations, which serves as the starting point for simulation or represents the experimental setup
- **`time`**: An array of time points at which measurements were taken (this can be None for datasets that only contain initial conditions)
- **`data`**: A dictionary that maps each species name to its complete concentration time series, providing the full temporal evolution of the system

### Simulation vs. Prediction

It's important to understand the distinction between simulation and prediction in Catalax, as these serve different purposes in the modeling workflow:

- **`model.simulate()`**: This method generates complete time-series data starting from initial conditions and integrating the differential equations forward in time. It's used when you want to create new datasets or explore system behavior under different conditions.
- **`model.predict()`**: This method evaluates the model at specific time points, typically matching experimental data points. It's used internally by the plotting system and for model comparison workflows.
