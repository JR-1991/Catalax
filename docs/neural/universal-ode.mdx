...
For datasets with noisy or irregular temporal sampling, you can additionally regularize training with temporal dropout (`temporal_dropout_p`) as described in [Temporal Dropout for Irregular-Time Robustness](./neural-regularization#temporal-dropout-for-irregular-time-robustness).
...
trained = universal_ode.train(
    dataset=simulated,
    strategy=strategy,
    temporal_dropout_p=0.1,  # Optional temporal regularization
)
```
...
- **Temporal dropout**: For noisy trajectories, add `temporal_dropout_p` and calibrate against validation behavior
